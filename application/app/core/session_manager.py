...
class SessionManager:
    ...
    def retrieve_session_info(self, session_id=None):
        if session_id is None:
            result = {}
            for sid in self.session_info:
                result[sid] = {
                    "info": self.session_info[sid],
                    "data": self.session_data.get(sid, {})
                }
            return result
    ...
    def save_data_to_session(self , session_id: str, key: str, data: Union[Dict[str, Any], pd.DataFrame, str, Any]):
    ...
    def fetch_session_data(self, session_id, key = None):
        ...
        data = self.session_data[session_id].get(key, [])

        # if isinstance(data, list) and len(data) > 0:
        #     # For a list of DataFrames
        #     if all(isinstance(d, list) for d in data) and all(isinstance(d[0], dict) for d in data if d):
